...
class SMSMsg:
        def __init__(self, contact_name, timestamp, text, type_, extra):
...
class MMSMsg(SMSMsg):
        def __init__(self, contact_name = "", timestamp = 0, text = "", type_ = 1, extra = {}):
                SMSMsg.__init__(self, contact_name, timestamp, text, type_, extra)
                self.images = []
...
def parseConversations(root, conversations, users, base_path, carrier_number):
        ...
        for child in root:
                ...
                if child.tag == 'sms':
                        ...
                        save_msg = SMSMsg(name, date, body, type_, {})
                        try:
...
def main():
        ...
        for input in args.input:
                ...
                lxml_parser = XMLParser(huge_tree = True, recover = True, ns_clean = True, encoding = 'utf-8')
